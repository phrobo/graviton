...
#define GRAVITON_INTROSPECTION_CONTROL_GET_PRIVATE(obj) ( \
    G_TYPE_INSTANCE_GET_PRIVATE ((obj), GRAVITON_INTROSPECTION_CONTROL_TYPE, \
...
grab_service_arg (GravitonIntrospectionControl *self,
                  GHashTable *args,
                  GError **error)
{
  GravitonService *subservice;
  const gchar *service_name = NULL;
  GravitonRootService *plugins = graviton_server_get_root_service (
    self->priv->server);

  if (g_hash_table_lookup (args, "service")) {
    GVariant *service_arg = g_hash_table_lookup (args, "service");
    if (g_variant_is_of_type (service_arg, G_VARIANT_TYPE_STRING))
      service_name = g_variant_get_string (service_arg, NULL);
  }

  if (service_name) {
    g_debug ("Querying service %s", service_name);
    subservice = graviton_service_get_subservice (GRAVITON_SERVICE (
              ...
              GParamSpec *pspec)
{
  ...
  case PROP_NODE_ID:
    g_value_set_string (value,
                        graviton_server_get_node_id (self->priv->server));
    break;
  case PROP_CLOUD_ID:
    g_value_set_string (value,
                        graviton_server_get_cloud_id (self->priv->server));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
    break;
...
static GVariant *
cb_properties(GravitonService *service,
              GHashTable *args,
              GError **error,
              gpointer user_data)
{
...
static GVariant *
cb_get_property (GravitonService *service,
                 GHashTable *args,
                 GError **error,
                 gpointer user_data)
{
  ...
  property_name = g_variant_get_string (g_hash_table_lookup (args,
                                                             "property"), NULL);
  GParamSpec *property =
    g_object_class_find_property (G_OBJECT_GET_CLASS (subservice),
                                  property_name);
  if (property) {
    GValue property_value = G_VALUE_INIT;
    GVariant *converted_variant = NULL;
    g_value_init (&property_value, property->value_type);
    g_object_get_property (G_OBJECT(subservice), property->name,
                           &property_value);
    if (G_VALUE_HOLDS_STRING (&property_value)) {
      converted_variant =
        g_variant_new_string (g_value_get_string (&property_value));
...
static GVariant *
cb_nodes(GravitonService *service,
         GHashTable *args,
         GError **error,
         gpointer user_data)
{
...
static GVariant *
cb_clouds(GravitonService *service,
          GHashTable *args,
          GError **error,
          gpointer user_data)
{
...
static GVariant *
cb_services(GravitonService *service,
            GHashTable *args,
            GError **error,
            gpointer user_data)
{
...
static GVariant *
cb_methods(GravitonService *service,
           GHashTable *args,
           GError **error,
           gpointer user_data)
{
...
graviton_internal_plugin_init (GravitonIntrospectionControl *self)
{
  ...
  GravitonService *introspection = g_object_new (GRAVITON_SERVICE_TYPE,
                                                 "name",
                                                 "introspection",
                                                 NULL);
